...
**Selected because:** oldest open issue (no labels, no comments, one-line description). Prior refinement (2026-08-13) identified this as verify-and-close; re-verified on current main (v0.9.0).
...
- `npm test` — **114/114 pass** on branch `cursor/single-task-refinement-a59e` (main @ v0.9.0).
...
- `q` and `Esc` exit paths call the same `cleanup()` function (code review); dedicated lifecycle tests for those keys remain optional hardening, not a blocker.
...
**Close the issue** — implementation landed in `f3083e2`; automated verification passes on 2026-08-19. Remaining work is optional `q`/`Esc` lifecycle tests and a quick manual terminal check.
