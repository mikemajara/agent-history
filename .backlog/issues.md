<!-- GENERATED FROM GITHUB ISSUES. DO NOT EDIT DIRECTLY. -->

# GitHub Issues

Generated at: 2026-08-11T22:45:16.002Z

| Issue | Status | Title | Labels | Assignees | Updated |
| --- | --- | --- | --- | --- | --- |
| #17 | OPEN | [Open new agent session from TUI (Ctrl+n / Alt+Enter)](https://github.com/mikemajara/agent-history/issues/17) | `agent-ready`, `priority:medium`, `status:ready`, `type:feat` | - | 2026-08-11 |
| #16 | OPEN | [Keycap-styled TUI footer shortcuts](https://github.com/mikemajara/agent-history/issues/16) | `agent-ready`, `priority:low`, `status:ready`, `type:nit` | - | 2026-08-11 |
| #15 | OPEN | [In-query dir:/date: filters; keep search generic](https://github.com/mikemajara/agent-history/issues/15) | `agent-ready`, `priority:high`, `status:ready`, `type:feat` | - | 2026-08-11 |
| #14 | OPEN | [Search-first TUI input chrome with teaching placeholder](https://github.com/mikemajara/agent-history/issues/14) | `agent-ready`, `priority:medium`, `status:ready`, `type:nit` | - | 2026-08-11 |
| #13 | OPEN | [Add directory column to compact TUI rows](https://github.com/mikemajara/agent-history/issues/13) | `agent-ready`, `priority:medium`, `status:doing`, `type:feat` | - | 2026-08-11 |
| #12 | OPEN | [Color-code agent badges in TUI session rows](https://github.com/mikemajara/agent-history/issues/12) | `agent-ready`, `priority:medium`, `status:ready`, `type:nit` | - | 2026-08-11 |
| #11 | OPEN | [Highlight search matches in the TUI preview pane](https://github.com/mikemajara/agent-history/issues/11) | `agent-ready`, `priority:medium`, `status:ready`, `type:feat` | - | 2026-08-11 |
| #10 | OPEN | [Add comfortable-density session rows](https://github.com/mikemajara/agent-history/issues/10) | - | - | 2026-07-28 |
| #9 | OPEN | [Add a scrollable session transcript view](https://github.com/mikemajara/agent-history/issues/9) | - | - | 2026-07-28 |
| #8 | OPEN | [When quitting agent history the terminal should clear](https://github.com/mikemajara/agent-history/issues/8) | - | - | 2026-06-14 |
| #6 | CLOSED | [FIX: Cursor agent prints the wrong string to output](https://github.com/mikemajara/agent-history/issues/6) | `priority:high`, `status:ready` | - | 2026-06-15 |
| #5 | OPEN | [Harden provider parsers for malformed JSONL](https://github.com/mikemajara/agent-history/issues/5) | `agent-ready`, `priority:low`, `status:ready`, `type:fix` | - | 2026-05-30 |
| #4 | OPEN | [Add no-preview mode for sensitive sessions](https://github.com/mikemajara/agent-history/issues/4) | `agent-ready`, `priority:low`, `status:ready`, `type:feat` | - | 2026-05-30 |
| #3 | OPEN | [Cache session index for large histories](https://github.com/mikemajara/agent-history/issues/3) | `agent-ready`, `priority:medium`, `status:ready`, `type:chore` | - | 2026-05-30 |
| #2 | OPEN | [Always-on TUI preview pane (side/bottom)](https://github.com/mikemajara/agent-history/issues/2) | `agent-ready`, `priority:medium`, `status:ready`, `type:feat` | - | 2026-08-11 |
| #1 | CLOSED | [Decide safe handoff for selected session command](https://github.com/mikemajara/agent-history/issues/1) | `priority:medium`, `status:duplicate`, `type:feat` | - | 2026-08-11 |
